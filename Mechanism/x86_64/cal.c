...
#define _ion_ical	*_ppvar[4]._pval
#define _ion_dicaldv	*_ppvar[5]._pval
...
static void nrn_alloc(Prop* _prop) {
 ...
 prop_ion = need_memb(_cal_sym);
 	_ppvar[4]._pval = &prop_ion->param[3]; /* ical */
 	_ppvar[5]._pval = &prop_ion->param[4]; /* _ion_dicaldv */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 6, "cvodeieq");
 	...
 	ivoc_help("help ?1 cal /Users/danielchapman/PythonDev/Final/Mechanism/x86_64/cal.mod\n");
...
static void _ode_matsol(_NrnThread* _nt, _Memb_list* _ml, int _type) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   ...
   nrn_update_ion_pointer(_cal_sym, _ppvar, 4, 3);
   nrn_update_ion_pointer(_cal_sym, _ppvar, 5, 4);
 }
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   ...
   ica = ( gcalbar * po + s * s * 8.0 * gcalbar ) * ( v - eca ) ;
   ical = ica ;
   }
 _current += ica;
 _current += ical;
 ...
 v=_v;
{
  ...
  eca = _ion_eca;
 { error =  states();
 if(error){fprintf(stderr,"at line 69 in file cal.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }  }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/danielchapman/PythonDev/Final/Mechanism/cal.mod";
static const char* nmodl_file_text = 
